...
def fun(arr):
    ...
    for j in range(len(arr) - 1):
        r1.append((f(arr[j + 1]) - f(arr[j])) / (arr[j + 1] - arr[j]))

    for j in range(len(arr) - 2):
        r2.append((r1[j + 1] - r1[j]) / (arr[j + 2] - arr[j]))

    for j in range(len(arr) - 3):
        r3.append((r2[j + 1] - r2[j]) / (arr[j + 3] - arr[j]))
    return r1, r2, r3
'''
...
def estimate_mod_der(n, x, nodes):
    ...
    for i in range(n + 1):
        ...
        for k in np.arange(c+step, d, step):
            modules[i] = max(abs(derivative(f, k, dx=1.0, n=i + 1, order = n+2)), abs(derivative(f, k-step, dx=1.0, n=i + 1, order = n+2)))
    return modules
...
print('\n', values)
print('\n')
print(actual_err(x,f,values))
print(estimate_mod_der(n,x,xx))
